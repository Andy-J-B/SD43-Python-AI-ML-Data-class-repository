...
# ----------------------------------------------------------------
def minimax(
    board: Board,  # current game board (2‑D list of squares)
    ...
    """
    Recursive evaluation of the game tree.
    Returns a numeric score representing how good the current board is
    for the AI. Higher scores are better for the AI.
    """
    # ------------------------------------------------------------
    # 1️⃣  Check if the game is already finished.
    # ------------------------------------------------------------
    winner = check_winner(board)  # does someone already have 3 in a row?
    if winner == ai_player:  # AI won → best possible outcome
        return 10 - depth  # faster win = higher score
    if winner == human_player:  # Human won → worst outcome
        return depth - 10  # slower loss = less terrible
    if is_draw(board):  # board full & no winner
        return 0  # perfectly neutral

    # ------------------------------------------------------------
    # 2️⃣  If the game is not over, explore the next moves.
    # ------------------------------------------------------------
    if is_maximizing:  # ---- AI's turn (we want to maximise)
        best = -float("inf")  # start with the worst possible score
        for r, c in get_available_moves(board):  # loop over every empty cell
            board[r][c] = ai_player  # make a provisional move for AI
            # recurse: now it will be the human's turn (is_maximizing=False)
            score = minimax(board, depth + 1, False, ai_player, human_player)
            board[r][c] = None  # undo the provisional move (backtrack)
            best = max(best, score)  # keep the highest score we can achieve
        return best  # return the best score found

    else:  # ---- Human's turn (we want to minimise)
        best = float("inf")  # start with the worst possible score for AI
        for r, c in get_available_moves(board):
            board[r][c] = human_player  # provisional move for the opponent
            # recurse: now it will be the AI's turn (is_maximizing=True)
            score = minimax(board, depth + 1, True, ai_player, human_player)
            board[r][c] = None  # undo the move (backtrack)
            best = min(best, score)  # keep the lowest score (human tries to hurt AI)
        return best  # return the worst‑for‑AI score we found
...
def ai_move(board: Board, ai_player: str, human_player: str) -> Move:
    """
    Choose the best move for the AI by evaluating every legal move
    with the minimax function defined above.
    """
    best_score = -float("inf")  # initialise with the worst possible score
    best_move: Move = (-1, -1)  # placeholder for the best coordinates

    # ------------------------------------------------------------
    # Try each legal move, score it, and keep the one with the highest score.
    # ------------------------------------------------------------
    for r, c in get_available_moves(board):
        board[r][c] = ai_player  # simulate AI playing in this cell
